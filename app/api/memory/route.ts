...
export async function GET(request: NextRequest) {
  ...
  const cacheKey = `memory_${lat.toFixed(4)}_${lng.toFixed(4)}_p${page}_l${limit}`

  const cached = getCache<any[]>(cacheKey)
  if (cached) {
    return NextResponse.json(cached.map((m) => ({
      ...m,
  ...
  const mapped = memories.map((m) => ({
    ...
    contributor_hash: m.contributor_hash,
  }))

  setCache(cacheKey, mapped, 15000)

  return NextResponse.json(mapped.map((m) => ({
    ...m,
    is_mine: myHash ? m.contributor_hash === myHash : false,
    contributor_hash: undefined,
  })))
