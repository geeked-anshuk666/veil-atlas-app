...
export async function GET(request: NextRequest) {
  ...
  const cacheKey = `feel_pins_${lat.toFixed(4)}_${lng.toFixed(4)}_p${page}_l${limit}`
  ...
  const mapped = pins.map((p) => ({
    ...
    contributor_hash: p.contributor_hash,
  }))

  // Cache database results for 15 seconds
...
export async function POST(request: NextRequest) {
  try {
    ...
    `
    // Invalidate feel pins caches
    clearCache('feel_pins_')
